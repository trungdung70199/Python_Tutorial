...
class GitHacker:
    ...
    def add_folder(self, base_url, folder):
        ...
        soup = bs4.BeautifulSoup(requests.get(url, verify=self.verify).text, features="html.parser")
        ...
        for link in links:
            ...
            if href.endswith("/"):
                ...
                if file_url.startswith(self.url):
                    ...
                    filepath = file_url[url_length:].strip().replace("..", "").split("/")
        ...
        with open(os.path.join(self.temp_dst, os.path.sep.join([".git", "HEAD"])), "r") as f:
            for line in f:
                if line.startswith("ref: "):
                    ...
                    file_path = os.path.join(self.temp_dst, os.path.sep.join([".git", "logs", ref_path]))
            ...
            for major in range(self.max_semanic_version):
                for minor in range(self.max_semanic_version):
                    for patch in range(self.max_semanic_version):
                        self.default_git_files.append([".git", "refs", "tags", f"v{major}.{minor}.{patch}"])
                        self.default_git_files.append([".git", "refs", "tags", f"{major}.{minor}.{patch}"])
        else:
